...
Exercise 1: id -un (whoami) Print the effective user ID of current user. Other alternative is whoami

Exercise 2: id -Gn (groups) Prints all the groups the current user is part of.
...
Exercise 11: chmod -R +X (chmod -R ugo+x */) Add execute permission to all subdirectories of the current directory for the everyone. Regular files should not be changed.

Exercise 12: mkdir -m751 my_dir Create a directory called dir_holberton with permissions 751 in the working directory. User has all read, write, and execute permissions. Group has read and execute permissions. Others have just execute permission.
